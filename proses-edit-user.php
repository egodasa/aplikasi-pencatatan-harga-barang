...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
  $query = $db->prepare("UPDATE tbl_user SET password = md5(?), nama_lengkap = ?, alamat = ?, level = ?, id_kecamatan = ? WHERE id_user = ?");
  ...
  $query->bindParam(5, $_POST['id_kecamatan']);
  $query->bindParam(6, $_POST['id_user']);
  $query->execute();
}
